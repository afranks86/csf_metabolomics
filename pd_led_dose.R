...
#   p_factors_panuc[no_penalty_index] <- 0
#   
#   #grouped = false is already enforced for small folds. I'm just writing it explicitly to avoid the warning.
#   binary_led_fit <- glmnet(features, labels, family = 'binomial', nlambda = 200, 
#                            alpha = .5, standardize = TRUE, penalty.factor = p_factors_panuc)
#   
...
# c_led_logistic <- purrr::map(1:5, ~logistic_control_analysis(imputed_c_untargeted5_binary_led, varname = "led", imp_num = .x, nlambda = 200))
...
untargeted_led_pred_df <- imputation_df(untargeted_led_analysis) %>%
  left_join(select(wide_data_untargeted_panuc, Id, GBAStatus), by = c("id" = "Id"))
(untargeted_led_predtruth <- predtruth_plot(untargeted_led_pred_df, name = "Untargeted led", position = "botright")) 
...
predtruth_plot(untargeted_led_pred_df, color = "apoe", errorbar = FALSE, name = "Untargeted", position = "botright")
predtruth_plot(untargeted_led_pred_df, color = "apoe4", errorbar = FALSE, name = "Untargeted", position = "botright")
predtruth_plot(untargeted_led_pred_df, color = "GBAStatus", errorbar = FALSE, name = "Untargeted", position = "botright") + 
  labs(x = "True dose",
       y = "Predicted dose",
       title = "PD: True vs Predicted Levodopa dose")
...
targeted_led_pred_df <- imputation_df(targeted_led_analysis) %>%
  left_join(select(wide_data_targeted_panuc, Id, GBAStatus), by = c("id" = "Id"))
(targeted_led_predtruth <- predtruth_plot(targeted_led_pred_df, name = "targeted led", position = "botright")) 
...
#plot colored by different things
predtruth_plot(targeted_led_pred_df, color = "apoe", errorbar = FALSE, name = "targeted", position = "botright")
predtruth_plot(targeted_led_pred_df, color = "apoe4", errorbar = FALSE, name = "targeted", position = "botright")
predtruth_plot(targeted_led_pred_df, color = "GBAStatus", errorbar = FALSE, name = "targeted", position = "botright")
